...
export default function NaadPass({ user, naadId }: NaadPassProps) {
	const formattedNaadId = naadId 
		? `NAAD-26-${naadId.toString().padStart(4, "0")}`
		: "PENDING";
				...
				<div className="relative p-6 flex flex-col gap-6">
					<div className="flex justify-between items-start">
						<div className="flex flex-col">
							<h3 className="text-2xl font-black italic tracking-tighter text-white">NAAD &apos;26</h3>
					...
					<div className="flex items-center gap-4 py-4 border-t border-white/10 border-b">
						{user.profileImageUrl ? (
							...
							<Image src={user.profileImageUrl} alt="Profile" width={64} height={64} className="w-16 h-16 rounded-full border-2 border-white/10 object-cover" />
						) : (
					...
					<div className="flex justify-between items-end">
						<div className="flex flex-col gap-1">
							...
							<span className={`font-mono text-xl font-bold tracking-wider drop-shadow-[0_0_8px_rgba(59,130,246,0.5)] ${
								naadId ? 'text-blue-400 shadow-blue-500/20' : 'text-zinc-600'
							}`}>
								{formattedNaadId}
							</span>
						</div>
